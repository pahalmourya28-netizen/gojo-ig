...
if sys.platform != "win32":
    ...
    pty = None  # Prevents NameError if referenced later
...
# Fix Windows cp1252 Unicode logging errors by forcing utf-8 on console
_console_handler = logging.StreamHandler()
_console_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
# Force utf-8 on the underlying stream to avoid cp1252 encode errors
...
OWNER_TG_ID = 8223027307
BOT_TOKEN = "8988013142:AAFaPQFwZkRr9TpaEiBZpzlhFdXFG3r0nic"
...
async def playwright_login_and_save_state(username: str, password: str, user_id: int) -> str:
    ...
    async with async_playwright() as p:
        ...
        login_url = "https://www.instagram.com/accounts/seal/?__coig_login=1"
...
def perform_login(page, username, password):
    try:
        ...
        try:
            page.wait_for_url(lambda url: 'accounts/seal' not in url and 'challenge' not in url and 'two_factor' not in url, timeout=60000)
...
🚀 Welcome to Gojo Satoru 's Domain Expansion  

💡 Type /technique to explore all commands  
...
✨ 𝗕𝗔𝗦𝗜𝗖  
┃ /technique 🆘 → Show this menu  
┃ /energy 📊 → System usage stats  

━━━━━━━━━━━━━━━━━━━━  

🔐 𝗟𝗢𝗚𝗜𝗡 𝗠𝗘𝗧𝗛𝗢𝗗𝗦  
┃ /seal 📱 → Normal login  
┃ /infinity 🌐 → Browser login  
┃ /sixeyes 🔑 → Session ID login  
┃ /barrier 🗝️ → Browser session ID  

━━━━━━━━━━━━━━━━━━━━  

👤 𝗔𝗖𝗖𝗢𝗨𝗡𝗧 𝗠𝗔𝗡𝗔𝗚𝗘𝗠𝗘𝗡𝗧  
┃ /tools 👀 → View accounts  
┃ /primary 🔄 <number> → Set default  
┃ /unseal 🚪 <username> → Remove account  
┃ /void 🛑 → Kill session  

━━━━━━━━━━━━━━━━━━━━  

🔄 𝗥𝗢𝗧𝗔𝗧𝗜𝗢𝗡 & 𝗦𝗘𝗧𝗨𝗣  
┃ /squad 📦 ig1-ig2 → Pair accounts  
┃ /solo ✨ → Remove pairing  
┃ /shift ⏱️ <min> → Switch interval  
┃ /clones 🔢 <1-5> → Set threads  
┃ /scan ⚙️ → Preferences  
...
👑 𝗔𝗗𝗠𝗜𝗡 𝗖𝗢𝗡𝗧𝗥𝗢𝗟𝗦  
┃ /recruit ➕ <tg_id> → Add user  
┃ /exile ➖ <tg_id> → Remove user  
┃ /sorcerers 📜 → List users  
┃ /purge 🧹 → Stop all tasks  

"""


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(DOMAIN_TEXT)
...
async def login_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user_id = update.effective_user.id
    if not is_authorized(user_id):
        await update.message.reply_text("⚠️ You are not authorised to use, dm owner to gain access! @Gojo Satoru  ⚠️")
        return ConversationHandler.END
...
async def get_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with SESSIONS_LOCK:
        if user_id in SESSIONS:
            await update.message.reply_text("⚠️ PTY session already running. Use /void first.")
            return ConversationHandler.END
...
# --- /infinity handlers (ASYNC, NO THREAD) ---
async def plogin_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("🔐 Enter Instagram username for Playwright seal: ")
    return PLO_USERNAME
...
async def plogin_get_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("🔄 Starting Playwright seal... (async, no threads)")
    ...
    try:
        ...
        await update.message.reply_text("✅ Playwright seal successful! Sessions saved. 🎉")
        ...
        await update.message.reply_text(f"❌ Seal failed: {err_msg}")

    except Exception as e:
        logging.exception("[PLOGIN] Unexpected exception in infinity_get_password")
...
async def viewmyac(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if user_id not in users_data:
        await update.message.reply_text("❌ You haven't saved any account. Use /seal to save one. ❌")
        return
    data = users_data[user_id]
    msg = "👀 Your saved cursed tools 👀\n"
    for i, acc in enumerate(data['accounts']):
        default = " (primary) ⭐" if data['default'] == i else ""
...
async def setig(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("❗ Usage: /primary <number> ❗")
        return
    ...
    await update.message.reply_text(f"✅ {num+1}. {acc} now is your primary account. ⭐")
...
async def logout_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if not context.args:
        await update.message.reply_text("❗ Usage: /unseal <username> ❗")
        return
    ...
    await update.message.reply_text(f"✅ Unsealed and removed {username}. Files deleted. ✅")
...
async def pair_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if not context.args:
        await update.message.reply_text("❗ Usage: /squad iguser1-iguser2-iguser3 ❗")
        return
    ...
    if user_id not in users_data or not users_data[user_id]['accounts']:
        await update.message.reply_text("❌ No accounts saved. Use /seal first. ❌")
        return
    data = users_data[user_id]
    accounts_set = {acc['ig_username'] for acc in data['accounts']}
    missing = [u for u in us if u not in accounts_set]
    if missing:
        await update.message.reply_text(f"⚠️ Can't find that account: {missing[0]}. Save it again with /seal. ⚠️")
        return
...
async def unpair_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users_data or not users_data[user_id].get('pairs'):
        await update.message.reply_text("❌ No active pair found. Use /squad first. ❌")
        return
    ...
    # --- no arguments case ---
    if not context.args:
        msg = "👥 Current squad list:\n"
        for i, u in enumerate(pair_list, 1):
            mark = "⭐" if i - 1 == pair_info.get('default_index', 0) else ""
            msg += f"{i}. {u} {mark}\n"
        msg += "\nUse `/solo all` to remove all pairs or `/solo <username>` to remove one."
        await update.message.reply_text(msg)
        return
    ...
    if arg == "all":
        data['pairs'] = None
        save_user_data(user_id, data)
        await update.message.reply_text("🧹 All squads removed successfully.")
        return
    ...
    if not pair_list:
        ...
        msg = f"✅ Removed {target}. Remaining squad: {', '.join(pair_list)}"
...
async def switch_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("❗ Usage: /shift <minutes> ❗")
        return
    min_ = int(context.args[0])
    data = users_data[user_id]
    if not data.get('pairs') or len(data['pairs']['list']) < 2:
        await update.message.reply_text("⚠️ No pair found. Use /squad first. ⚠️")
        return
    if min_ < 5:
    ...
    save_user_data(user_id, data)
    await update.message.reply_text(f"⏱️ Shift interval set to {min_} minutes.")
...
async def threads_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("❗ Usage: /clones <1-5> ❗")
        return
    n = int(context.args[0])
    if n < 1 or n > 5:
        await update.message.reply_text("⚠️ clones must be between 1 and 5. ⚠️")
        return
    if user_id not in users_data:
        users_data[user_id] = {'accounts': [], 'default': None, 'pairs': None, 'switch_minutes': 10, 'threads': 1}
        save_user_data(user_id, users_data[user_id])
    ...
    data['threads'] = n
    save_user_data(user_id, data)
    await update.message.reply_text(f"🔁 Clones set to {n}.")
...
async def viewpref(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if user_id not in users_data:
        await update.message.reply_text("❌ No data. Use /seal. ❌")
        return
    data = users_data[user_id]
    saved_accounts = ', '.join([acc['ig_username'] for acc in data['accounts']])
    msg = "🔧 Your domain preferences:\n"
    if data.get('pairs'):
        pl = data['pairs']['list']
        msg += f"Squad: yes — {len(pl)} accounts\n"
    ...
    msg += f"⏱️ Shift interval: {switch_min} minutes\n"
    threads = data.get('threads', 1)
    msg += f"🧵 Clones: {threads}\n"
    msg += f"👤 Saved tools: {saved_accounts}\n"
    ...
    if running_attacks:
        ...
        msg += f"\nActive technique PID {pid} ({disp})\n"
        msg += "Technique active...!\n"
...
async def attack_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_authorized(user_id):
        ...
        return ConversationHandler.END
    if user_id not in users_data or not users_data[user_id]['accounts']:
        await update.message.reply_text("❗ Please /seal first. ❗")
        return ConversationHandler.END
    data = users_data[user_id]
...
async def get_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if 'dm' in text:
        ...
        # ✅ Agar beech me /abort aaya tha to result ignore karo
...
async def get_target_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not thread_url:
        await update.message.reply_text("❌ Could not lock thread id with primary account.")
        return ConversationHandler.END
...
async def get_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(pair_list) == 1:
        warning = "⚠️ Warning: You may get chat ban if you use a single account too long. Use /squad to make multi-account rotation.\n\n"
    else:
        warning = ""
    ...
    if len(running_msg) >= 5:
        await update.message.reply_text("⚠️ Max 5 techniques running. Cancel one first. ⚠️")
        if os.path.exists(names_file):
            os.remove(names_file)
        return ConversationHandler.END
    ...
    for u in pair_list:
        if u == curr_u:
            ...
            status += f"cooldown - {u}\n"
    status += f"To stop 🛑 type /abort {task['display_pid']} or /abort all to stop all processes."
...
def switch_task_sync(task: Dict):
    ...
    try:
        ...
        text = "Technique active...!\n" + get_switch_update(task)
        text += f"\nTo stop 🛑 type /abort {task['display_pid']} or /abort all to stop all processes."
        if msg_id:
            asyncio.run_coroutine_threadsafe(
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if not context.args:
        await update.message.reply_text("❗ Usage: /abort <PID> or /abort all ❗")
        return
    arg = context.args[0]
    ...
    if arg == 'all':
        ...
        for task in tasks[:]:
            ...
            tasks.remove(task)
            stopped_count += 1
        await update.message.reply_text(f"🛑 Cancelled all your techniques! ({stopped_count}) 🛑")
    elif arg.isdigit():
        pid_to_stop = int(arg)
        stopped_task = None
        ...
        for task in tasks[:]:
            if task.get('display_pid') == pid_to_stop:
                ...
                stopped_task = task
                tasks.remove(task)
                await update.message.reply_text(f"🛑 Cancelled technique {pid_to_stop}!")
                break
...
async def task_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized(user_id):
        ...
        return
    if user_id not in users_tasks or not users_tasks[user_id]:
        await update.message.reply_text("❌ No active techniques. ❌")
        return
    tasks = users_tasks[user_id]
    active_tasks = []
    for t in tasks:
...
async def cancel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in user_fetching:
        user_fetching.discard(user_id)
        await update.message.reply_text("❌ Fetching aborted! 🚫")
    else:
        await update.message.reply_text("No active fetching to abort. 😊")
...
async def add_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 1:
        await update.message.reply_text("❗ Usage: /recruit <tg_id> ❗")
        return
    try:
        tg_id = int(context.args[0])
        if any(u['id'] == tg_id for u in authorized_users):
            await update.message.reply_text("❗ Sorcerer already recruited. ❗")
            return
        authorized_users.append({'id': tg_id, 'username': ''})
        save_authorized()
        await update.message.reply_text(f"➕ Recruited {tg_id} as sorcerer. ➕")
    except:
        await update.message.reply_text("⚠️ Invalid sorcerer ID. ⚠️")
...
async def remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_owner(user_id):
        ...
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("❗ Usage: /exile <tg_id> ❗")
        return
    tg_id = int(context.args[0])
    global authorized_users
    authorized_users = [u for u in authorized_users if u['id'] != tg_id]
    save_authorized()
    await update.message.reply_text(f"➖ Exiled {tg_id} from sorcerers. ➖")
...
async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not authorized_users:
        await update.message.reply_text("❌ No sorcerers registered. ❌")
        return
    msg = "📜 Active sorcerers: 📜\n"
...
def main_bot():
    ...
    # Check if another instance is already running by trying to delete webhook
    import asyncio
    try:
    ...
    application.add_handler(CommandHandler("domain", start))
    application.add_handler(CommandHandler("start", start))  # backward compat
    application.add_handler(CommandHandler("technique", help_command))
    application.add_handler(CommandHandler("help", help_command))  # backward compat
    application.add_handler(CommandHandler("tools", viewmyac))
    application.add_handler(CommandHandler("primary", setig))
    application.add_handler(CommandHandler("squad", pair_command))
    application.add_handler(CommandHandler("solo", unpair_command))
    application.add_handler(CommandHandler("shift", switch_command))
    application.add_handler(CommandHandler("clones", threads_command))
    application.add_handler(CommandHandler("scan", viewpref))
    application.add_handler(CommandHandler("cancel", stop))
    application.add_handler(CommandHandler("active", task_command))
    application.add_handler(CommandHandler("recruit", add_user))
    application.add_handler(CommandHandler("exile", remove_user))
    application.add_handler(CommandHandler("sorcerers", list_users))
    application.add_handler(CommandHandler("unseal", logout_command))
    application.add_handler(CommandHandler("void", cmd_kill))
    application.add_handler(CommandHandler("purge", flush))
    application.add_handler(CommandHandler("energy", usg_command))
    application.add_handler(CommandHandler("abort", cancel_handler))

    conv_login = ConversationHandler(
        entry_points=[CommandHandler("seal", login_start)],
        states={
    ...
    conv_plogin = ConversationHandler(
        entry_points=[CommandHandler("infinity", plogin_start)],
        states={
    ...
    conv_slogin = ConversationHandler(
        entry_points=[CommandHandler("sixeyes", slogin_start)],
        states={
    ...
    psid_handler = ConversationHandler(
        entry_points=[CommandHandler("barrier", psid_start)],
        states={
    ...
    conv_attack = ConversationHandler(
        entry_points=[CommandHandler("purple", attack_start), CommandHandler("attack", attack_start)],
        states={
    ...
    conv_pattack = ConversationHandler(
        entry_points=[CommandHandler("red", pattack_start)],
        states={
    ...
    # Error handler for telegram errors
    async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Log Errors caused by Updates."""
        from telegram.error import Conflict, NetworkError, TimedOut
        if isinstance(context.error, Conflict):
            logging.error("⚠️ Another bot instance is already running! Kill all python processes and restart.")
            return
        elif isinstance(context.error, (NetworkError, TimedOut)):
            logging.warning(f"Network issue: {context.error}")
            return
        logging.error(f"Exception while handling an update: {context.error}", exc_info=context.error)

    application.add_error_handler(error_handler)

    print("🚀 Domain Expansion starting with technique system!")
    # drop_pending_updates=True prevents old messages from flooding on restart
